...
- Updated primary navigation to Portfolio, Data & Protocol Hub, AI Coworker, Workflows, Banking Hands, Approvals, Runs, Audit, and Integrations, with operating mode and role context visible in the shell.
- Added missing `/audit`, `/integrations`, `/workflows`, `/coworker`, and `/protocol-hub` routes, and redirected `/` to the portfolio-first home experience.
...
- Added written system design coverage for Kyne's six Agent OS layers, architecture diagram, outreach lifecycle flow, backend services, entities, API table, REST vs SSE, scaling, deployment, RBAC, and security/compliance principles.
